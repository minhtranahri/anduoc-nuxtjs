$(document).ready(function () {
  ...
  $('.list-menu-res>.item-menu .mcon-angle-right').click(function (event) {
  ...
  $('#img-pro-full').slick({
    ...
    slidesToScroll: 1,
    arrows: false,
    asNavFor: '#img-pro-thumb'
  });
  $('#img-pro-thumb').slick({
    slidesToShow: 3,
    slidesToScroll: 1,
    asNavFor: '#img-pro-full',
    centerMode: true,
    focusOnSelect: true,
    adaptiveHeight: true,
  });
...
function filterAction(item, el) {
  ...
  if ($('#' + item).children().length == 0) {
    $.ajax({
      ...
      complete: function () {
        $('#' + item + ' .slicktest').slick({
          slidesToShow: 5,
          slidesToScroll: 5,
          prevArrow: '<button type="button" class="slick-prev-edit"><i class="mcon-angle-left"></i></button>',
          nextArrow: '<button type="button" class="slick-prev-edit right-slick"><i class="mcon-angle-right"></i></button>'
        });
      }
    });
  } else {
    ...
    setTimeout(function () {
      $('span.loading-content.display-slide').css('display', 'none');
    }, 500);
  }
}
...
function rmCart(id) {
  var rm = document.getElementById('remove-cart');
  var num = ($('#num_in_cart').attr('value'));
  var total = 0;
  $(".list_pro_item_" + id).css('display', 'none');
  $.ajax({
    url: 'delete_cart/' + id,
